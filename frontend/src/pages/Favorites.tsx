...
function loadViewerSettings(): ViewerSettings {
  ...
  return { theme: "auto", font: "sans", spacing: "normal", fontSize: 1 };
}
...
function LyricsViewer({
  ...
  const rootStyle = isAuto
    ? {
        background: "var(--color-surface-raised)",
        color: "var(--color-foreground)",
        fontFamily: fontStack,
      }
    : { background: theme.bg, color: theme.text, fontFamily: fontStack };

  const headerBorder = isAuto ? "var(--color-edge)" : theme.border;

  const settingsStyle = isAuto
    ? { background: "var(--color-surface)", borderColor: "var(--color-edge)" }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col" style={rootStyle}>
      {/* ── Header ── */}
      <div
      ...
      >
        ...
        >
          <button
            onClick={() =>
              set("fontSize", Math.max(0.65, +(s.fontSize - 0.15).toFixed(2)))
            }
            disabled={s.fontSize <= 0.65}
            aria-label="Verkleinern"
            className="w-7 h-7 flex items-center justify-center rounded-md text-xs font-bold disabled:opacity-25 transition-opacity"
          >
            A−
          </button>
          <button
            onClick={() => set("fontSize", 1)}
            className="px-1.5 h-7 flex items-center text-[11px] tabular-nums min-w-[36px] justify-center transition-opacity"
            style={{ opacity: 0.55 }}
          >
          ...
          <button
            onClick={() =>
              set("fontSize", Math.min(2.5, +(s.fontSize + 0.15).toFixed(2)))
            }
        ...
        <button
          ...
          className="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg transition-opacity"
      ...
      {showSettings && (
        <div
          className="flex-shrink-0 px-4 py-3.5 border-b space-y-3"
          style={settingsStyle}
        >
          {/* Theme */}
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-2xl mx-auto px-6 py-8 sm:px-10 sm:py-12">
          {song.lyrics ? (
            <p
              className="whitespace-pre-wrap"
              style={{ fontSize: `${s.fontSize}rem`, lineHeight }}
            >
              {song.lyrics}
            </p>
          ) : (
            <p className="text-sm py-16 text-center" style={{ opacity: 0.4 }}>
